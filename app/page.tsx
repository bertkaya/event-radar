...
export default function Home() {
  ...
  useEffect(() => { applyFilters() }, [activeCategory, sortBy, priceFilter, allEvents, favCounts])
    ...
    <div className="flex flex-col h-screen w-full bg-white text-black font-sans overflow-hidden">
          ...
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="p-2 max-h-[60vh] overflow-y-auto">
              {PRESET_LOCATIONS.map((loc) => (
                <button key={loc.name} onClick={() => handleSelectLocation(loc)} className="w-full text-left p-3 hover:bg-brand/5 rounded-lg text-sm font-medium text-gray-700 border-b border-gray-50 last:border-0 transition-colors">{loc.name}</button>
              ))}
            </div>
            <div className="p-4 bg-gray-50 border-t">
              <button onClick={() => { handleLocate(); setShowLocModal(false); }} className="w-full bg-black text-white py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2"><Navigation size={16}/> Konumumu Bul (GPS)</button>
            </div>
          </div>
        </div>
      )}
              ...
              <div className="flex flex-col gap-3 mb-6 bg-gray-50 p-4 rounded-xl border border-gray-100">
                 ...
                 <div className="flex items-center gap-3 text-gray-700">
                    ...
                    <div>
                        ...
                        {selectedEvent.address && <div className="text-xs text-gray-500 mt-1 leading-tight">{selectedEvent.address}</div>}
                    </div>
                 </div>
                 ...
                 <div className="flex items-center gap-3 text-gray-700">
                    ...
                    <div><div className="text-xs font-bold text-gray-400 uppercase">Favori</div><div className="font-bold text-lg">{favCounts[selectedEvent.id] || 0} Kişi</div></div>
                 </div>
              </div>
      ...
      <header className="h-[70px] bg-white border-b border-gray-200 px-4 md:px-6 flex justify-between items-center z-50 shrink-0 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => setShowLocModal(true)} className="flex items-center gap-1 text-sm font-bold text-gray-700 hover:bg-gray-100 px-3 py-1.5 rounded-full transition"><MapPin size={16} className="text-brand"/>{currentLocName}<ChevronDown size={14} className="text-gray-400"/></button>
        </div>
        <div className="flex items-center gap-4">
          {user ? (
            <div className="flex items-center gap-3">
              <Link href="/profile" className="text-right hidden md:block hover:opacity-70 transition cursor-pointer"><div className="text-xs font-bold text-gray-900">{user.email.split('@')[0]}</div><div className="text-[10px] text-gray-500 flex justify-end gap-1"><span>{favorites.length} Favori</span></div></Link>
      ...
      <div className="flex flex-1 flex-col md:flex-row overflow-hidden relative">
        <div className="h-[40%] md:h-full md:w-[60%] bg-gray-100 relative order-1 md:order-2">
          ...
          <div className="md:hidden absolute top-4 left-4 right-16 z-[900] overflow-x-auto no-scrollbar">
             <div className="flex gap-2">
               {categories.map(cat => (<button key={cat} onClick={() => setActiveCategory(cat)} className={`px-3 py-1.5 rounded-lg text-xs font-bold shadow-md whitespace-nowrap backdrop-blur-md ${activeCategory === cat ? 'bg-brand text-white' : 'bg-white/90 text-gray-800'}`}>{cat}</button>))}
             </div>
          </div>
        </div>
        ...
        <div className="h-[60%] md:h-full md:w-[40%] bg-white order-2 md:order-1 border-r border-gray-200 flex flex-col shadow-2xl relative z-20">
           <div className="p-4 border-b border-gray-100 bg-white shrink-0 space-y-3">
             <div className="flex justify-between items-center"><h1 className="text-2xl font-black tracking-tighter text-gray-900">AKIŞ</h1><div className="text-[10px] font-bold text-gray-400">{events.length} Etkinlik</div></div>
             <div className="flex gap-2 overflow-x-auto no-scrollbar">
                <select value={activeCategory} onChange={(e) => setActiveCategory(e.target.value)} className="bg-gray-100 text-xs font-bold p-2 rounded-lg border-none focus:ring-0 outline-none cursor-pointer"><option value="Tümü">Tüm Kategoriler</option>{Array.from(new Set(allEvents.map(e => e.category))).map(c => <option key={c} value={c}>{c}</option>)}</select>
                <button onClick={() => setSortBy(sortBy === 'date-asc' ? 'date-desc' : (sortBy === 'date-desc' ? 'popular' : 'date-asc'))} className="flex items-center gap-1 bg-gray-100 text-xs font-bold px-3 py-2 rounded-lg whitespace-nowrap"><ArrowUpDown size={14}/>{sortBy === 'date-asc' ? 'En Yakın' : (sortBy === 'date-desc' ? 'En Uzak' : 'Popüler')}</button>
                <button onClick={() => setPriceFilter(priceFilter === 'all' ? 'free' : 'all')} className={`flex items-center gap-1 text-xs font-bold px-3 py-2 rounded-lg whitespace-nowrap border ${priceFilter === 'free' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-white border-gray-200 text-gray-600'}`}><Banknote size={14}/>{priceFilter === 'free' ? 'Ücretsiz' : 'Tümü'}</button>
             </div>
           </div>
           ...
           <div className="flex-1 overflow-y-auto p-4 space-y-6 bg-gray-50">
             ...
             {events.map((event) => {
               ...
               return (
                <div key={event.id} onClick={() => setSelectedEvent(event)} className={`group bg-white rounded-3xl cursor-pointer transition-all border relative overflow-hidden flex flex-row md:flex-col items-stretch md:items-stretch h-32 md:h-auto hover:shadow-lg hover:border-brand/30 ${!event.image_url ? 'h-auto' : ''}`}>
                  {event.image_url && (
                  ...
                  <div className="p-3 md:p-4 flex-1 min-w-0 flex flex-col justify-between">
                    ...
                    <div>
                        <div className="flex items-center justify-between"><h3 className="font-bold text-sm md:text-lg text-gray-900 leading-tight line-clamp-2">{event.title}</h3>{isRecommended && !isSoldOut && <Star size={12} className="fill-yellow-400 text-yellow-400 shrink-0 ml-1"/>}</div>
                    ...
                    <div className="flex justify-between items-end mt-2">
                        ...
                        <button onClick={(e) => toggleFavorite(e, event.id)} className="p-1.5 rounded-full hover:bg-gray-100 text-gray-400 z-10 relative"><Heart size={16} className={isFav ? "fill-brand text-brand" : ""} /></button>
                    </div>
                  </div>
                </div>
               )
