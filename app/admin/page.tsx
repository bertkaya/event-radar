...
export default function Admin() {
  ...
  const handleEditClick = (event: any) => {
    ...
    setFormData({
      ...
      address: event.address || '', // Adres verisi
  ...
  const resetForm = () => {
    setFormData({ title: '', venue: '', address: '', category: 'Müzik', price: '', date: '', time: '', lat: '', lng: '', description: '', maps_url: '', image_url: '', ticket_url: '', sold_out: false })
  }
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      const payload = {
        ...
        address: formData.address, // Adres eklendi
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 font-sans text-gray-800">
      <div className="max-w-5xl mx-auto space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                   ...
                   <div className="space-y-2">
                     <input name="venue" value={formData.venue} onChange={handleChange} required className="w-full border p-3 rounded-lg" placeholder="Mekan Adı (Örn: Zorlu PSM)" />
                     <textarea name="address" value={formData.address} onChange={handleChange} rows={2} className="w-full border p-3 rounded-lg resize-none text-sm bg-gray-50" placeholder="Açık Adres (Örn: Levazım Mah. Koru Sok. No:2 Beşiktaş/İstanbul)"></textarea>
                   </div>
        ...
        <div className="bg-white rounded-2xl shadow border border-gray-200 overflow-hidden">
          <div className="bg-gray-100 p-4 font-bold text-gray-700 border-b flex justify-between items-center"><span>YÖNETİM LİSTESİ ({events.length})</span></div>
            ...
            {events.map(event => (
              <div key={event.id} className={`p-4 flex items-center justify-between hover:bg-gray-50 transition ${!event.is_approved ? 'bg-red-50 border-l-4 border-red-500' : ''}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-bold text-gray-900 flex items-center gap-2">{event.title} {!event.is_approved && <span className="bg-red-600 text-white text-[10px] px-2 py-0.5 rounded uppercase font-black">ONAYLA!</span>}{event.sold_out && <span className="bg-gray-800 text-white text-[10px] px-2 py-0.5 rounded uppercase">SOLD OUT</span>}</div>
                    <div className="text-xs text-gray-500 flex items-center gap-2 mt-1"><span className="flex items-center gap-1 text-brand font-bold"><Calendar size={12}/> {formatEuroDate(event.start_time)}</span><span className="flex items-center gap-1"><MapPin size={12}/> {event.venue_name}</span></div>
                  </div>
                </div>
                <div className="flex gap-2">
